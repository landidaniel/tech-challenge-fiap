...
from src.churn.config import (  # noqa: E402
    ARTIFACTS_DIR,
    COST_FN_DEFAULT,
    COST_FP_DEFAULT,
    DATA_PATH,
...
from src.churn.evaluate import evaluate, find_optimal_threshold, get_probs  # noqa: E402
from src.churn.model import ChurnMLP  # noqa: E402
from src.churn.pipeline import build_pipeline, save_artifacts  # noqa: E402
from src.churn.preprocessing import load_raw  # noqa: E402
from src.churn.train import train  # noqa: E402

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s %(levelname)s %(name)s — %(message)s",
)
_log = logging.getLogger("train_and_save")
...
def main(args: argparse.Namespace) -> None:
    ...
    _log.info("Dispositivo: %s", device)

    # 1. Dados
    _log.info("Carregando dados de: %s", args.data)
    X_df, y = load_raw(args.data)
    _log.info(
        "Dataset: %d amostras | %d colunas brutas | churn=%.1f%%",
        X_df.shape[0], X_df.shape[1], y.mean() * 100,
    )
    ...
    _log.info(
        "Splits — treino: %d | val: %d | teste: %d | features: %d",
    ...
    _log.info("Modelo: %s | dropout=%.1f | params=%d", hidden, args.dropout, total_params)  # noqa: E501

    # 6. Treinamento
    _log.info("Iniciando treinamento...")
    history = train(
        model, train_loader, val_loader, device,
        lr=args.lr, epochs=args.epochs, patience=args.patience,
        pos_weight=pos_weight,
    )
    _log.info("Treinamento concluido em %d epochs.", len(history["train_loss"]))
    ...
    for k, v in metrics.items():
        _log.info("  %-12s: %.4f", k, v)
    _log.info("  threshold   : %.4f", best_thresh)
    _log.info("  custo       : R$ %.2f", best_cost)

    # 8. Salvar artefatos
    save_artifacts(
        pipeline, model, threshold=best_thresh, out_dir=Path(args.artifacts_dir)
    )
    _log.info("Artefatos prontos para a API em: %s", args.artifacts_dir)
    ...
    if not args.no_mlflow:
        ...
        _log.info("Run registrado no MLflow.")
